        ...
        @if($orders->count() > 0)
            <div class="md:hidden divide-y divide-slate-100">
                @foreach($orders as $order)
                    <a href="{{ route('tailor.orders.show', $order) }}" class="block p-4 hover:bg-slate-50 transition-colors">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <span class="inline-flex font-mono text-xs font-semibold text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md">
                                    {{ $order->order_code }}
                                </span>
                        ...
                        <div class="mt-3 grid grid-cols-2 gap-2 text-xs">
                            <div class="rounded-lg bg-slate-50 px-3 py-2">
                                <p class="text-slate-400">Estimasi</p>
                                <p class="font-semibold text-slate-700">Rp {{ number_format($order->estimated_price, 0, ',', '.') }}</p>
                            </div>
                            <div class="rounded-lg bg-slate-50 px-3 py-2">
                                <p class="text-slate-400">Deadline</p>
                                <p class="font-semibold {{ $order->deadline && $order->deadline->isPast() && $order->status->value !== 'selesai' ? 'text-red-600' : 'text-slate-700' }}">
