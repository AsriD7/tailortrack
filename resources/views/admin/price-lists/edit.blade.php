        ...
        <form action="{{ route('admin.price-lists.update', $priceList) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none">{{ old('description', $priceList->description) }}</textarea>
            </div>

            <div class="pt-4 border-t border-slate-100 flex gap-3">
                <button type="submit" class="gradient-brand text-white px-6 py-2.5 rounded-lg text-sm font-semibold hover:opacity-90 transition-opacity">Simpan Perubahan</button>
