...
@section('page-subtitle', $priceList->name)
...
@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl shadow-soft border border-tailor-purple/10 p-8">
        <h2 class="text-lg font-bold text-slate-800 mb-6">Edit Layanan: {{ $priceList->name }}</h2>

        <form action="{{ route('admin.price-lists.update', $priceList) }}" method="POST" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $priceList->name) }}"
                    class="w-full px-4 py-2.5 border {{ $errors->has('name') ? 'border-red-400 bg-red-50' : 'border-slate-200' }} rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-tailor-gold focus:border-transparent">
                @error('name')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1.5">Kategori <span class="text-red-500">*</span></label>
                <select name="category" class="w-full px-4 py-2.5 border {{ $errors->has('category') ? 'border-red-400 bg-red-50' : 'border-slate-200' }} rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-tailor-gold focus:border-transparent bg-white">
                    @foreach(['Atasan','Bawahan','Terusan','Perbaikan','Lainnya'] as $cat)
                        <option value="{{ $cat }}" {{ old('category', $priceList->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="base_price" value="{{ old('base_price', $priceList->base_price) }}" min="0"
                        class="w-full pl-10 pr-4 py-2.5 border {{ $errors->has('base_price') ? 'border-red-400 bg-red-50' : 'border-slate-200' }} rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-tailor-gold focus:border-transparent">
                </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-tailor-gold focus:border-transparent resize-none">{{ old('description', $priceList->description) }}</textarea>
            </div>

            <div class="pt-4 border-t border-slate-100 flex gap-3">
                <button type="submit" class="brand-gradient text-white px-6 py-2.5 rounded-lg text-sm font-semibold hover:opacity-90 transition-opacity">Simpan Perubahan</button>
